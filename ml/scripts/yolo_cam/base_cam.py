...
from yolo_cam.utils.model_targets import ClassifierOutputTarget, YOLOBoxScoreTarget
...
class BaseCAM:
    def __init__(self,
        ...
        if self.uses_gradients and hasattr(actual_model, "requires_grad_"):
            actual_model.requires_grad_(True)
        self.activations_and_grads = ActivationsAndGradients(
            self.model, target_layers, reshape_transform)
        self.outputs = []

    def _extract_model_output(self, outputs):
    ...
    def _num_extra_channels(self) -> int:
...
#                                                    requires_grad=True)
        ...
        model_output = self._extract_model_output(outputs)
        self.outputs.append(model_output)
        if targets is None:
            if self.task == 'od' or self.task == 'seg':
                if torch.is_tensor(model_output):
                    targets = [YOLOBoxScoreTarget(num_extra_channels=self._num_extra_channels())]
                else:
                    targets = []
            elif self.task == 'cls':
                try:
                    target_categories = model_output[0].probs.top5
                except AttributeError:
                    if torch.is_tensor(model_output):
                        target_categories = [int(torch.argmax(model_output, dim=-1).flatten()[0].item())]
                    else:
                        target_categories = [0]
                targets = [ClassifierOutputTarget(category) for category in target_categories]
            else:
                print('Invalid Task Entered or no detections found for CAM')
                targets = []

        if self.uses_gradients:
            self.model.zero_grad()
            loss = sum([target(model_output) for target in targets])
